...
@AllArgsConstructor
public class ProjectRemovalHandler extends VelocityHandler {
    ...
    protected Template handleInternal(Request request, Response response, Map<String, Object> model) throws Exception {
        ...
        ValidatingCommand<Project> command = Command.projectRemovalCommand(projectsRepository, currentUserRepository);
        Command.CommandResult result = command.execute(project);
        if (result == Command.CommandResult.SUCCESS) {
            model.put("successMessages", Arrays.asList("Project removed successfully!"));
